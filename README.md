...
4. `결과물 저장하기`를 누르면 accessToken이 포함된 링크가 생성됩니다.
5. `/memory/[token]` 링크로 결과물을 다시 확인합니다.

## 저장 방식

- Supabase 환경값이 있으면 `memories` 테이블에 저장합니다.
- Supabase 환경값이 없으면 개발 서버 메모리의 mock DB에 임시 저장합니다.
- kioskMode에서는 사용자 입력, 결과물, 음성 데이터가 localStorage에 저장되지 않습니다.
- mock DB는 서버가 재시작되면 사라집니다. 실제 재열람 운영에는 Supabase가 필요합니다.
...
- [ ] 한국어 음성인식 결과가 textarea에 자동 입력됨
- [ ] 음성인식 결과를 사용자가 수정 가능
- [ ] 기록 완료 후 이메일 입력 화면이 표시됨
- [ ] 잘못된 이메일 형식은 저장 불가
- [ ] 결과물 저장 후 고유 링크가 표시됨
- [ ] `/memory/[token]` 접속 시 결과물 확인 가능
- [ ] 잘못된 token 접속 시 오류 메시지 표시
- [ ] 메인으로 돌아가기 클릭 시 초기화
- [ ] 5분 무입력 시 자동 초기화
- [ ] 모바일 Chrome에서 기본 흐름 확인
